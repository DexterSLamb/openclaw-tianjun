...
export function buildAgentSystemPrompt(params: {
  ...
  if (currentModel.startsWith("local/")) {
    ...
    const slimLines = [
      ...
      "",
      "## RULES (必须遵守)",
      "1. 对于常识性问题（如名人介绍、历史事件、科学知识），直接用你自己的知识回答，不要调用任何工具。",
      "2. 只有当用户问实时信息（天气、最新新闻、今日价格、最近比赛结果）时，才调用 web_search。",
      '3. web_search 工具可用且正常工作。不要说"无法搜索"或"缺少API密钥"。如果搜索返回错误，告诉用户搜索暂时失败。',
      "4. memory_search/memory_get 只用于查找之前对话中保存的信息，不要用来回答常识问题。",
      "",
      "## General Rules",
